...
class SquadBuilderGame extends StatefulWidget {
  SquadBuilderGame({Key? key}) : super(key: key);
...
class _SquadBuilderGameState extends State<SquadBuilderGame> {
  final Stream<QuerySnapshot> _playersStream = FirebaseFirestore.instance
      ...
      .where('subscriptions', arrayContains: gameId)
      .snapshots();
        ...
        actions: [
          ElevatedButton(
              onPressed: () {
                ...
                print(_SquadBuilderGameState.activeTargets[0].toString());
              },
        ...
        if (widget.activePlayer.uid == data?.uid) {
          setState(
            () {
              ...
              _SquadBuilderGameState.activeTargets[widget.arrayNum] = false;
              info['onBench'] = true;
              firebaseFirestore
                  .collection("users")
